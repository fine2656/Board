...
public class LargeThumbnailManager {
	...
	public String doCreateThumbnail(String filename, String path) 
		...
		if(image.exists()) {// 이미지가 존재한다면
		    Thumbnails.of(image).size(800, 800).outputFormat(fileExt.substring(1)).toFile(thumbnail);  // 이미지의 사이즈를 줄려주고, 밖으로 보내준다.
		    //fileExt.substring(1)).: 파알 확장자명 , toFile(thumbnail)  : 파일을 내보낸다
		}
