...
public class FileManager {
	...
	public String doFileUpload(byte[] bytes, String originalFilename, String path) throws Exception {
		...
		newFilename = String.format("%1$tY%1$tm%1$td%1$tH%1$tM%1$tS",  // 년원일 시분초 나노초
				         Calendar.getInstance());
		newFilename += System.nanoTime();
		newFilename += fileExt;// 파일 확장자
		
		// 업로드할 경로가 존재하지 않는 경우 폴더를 생성 한다.
		File dir = new File(path);
		if(!dir.exists())// 폴더가 존재하지 않으면
			dir.mkdirs();//폴더가 존재하지 않으면 폴더를 만든다
		...
		return newFilename; // 파일을 업로드 해주고, 업로드 된 파일명을 리턴해준다.
	}
	    ...
	    try {
	        ...
	        if (file.exists()){
	          ...
	          // 즉, ServletOutputStream 은 파일 다운로드에 사용되는 출력 스트림용 클래스이다. 
	   			...
	   			int length = 0;
	    		while ((length = fin.read(readByte, 0, 4096)) != -1) { // 마지막이 아니라면( != -1) 끝까지 읽어준다
